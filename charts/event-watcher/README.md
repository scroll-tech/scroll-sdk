...
| command[1] | string | `"-c"` |  |
| command[2] | string | `"event_watcher --config /app/conf/rollup-config.json --metrics --metrics.addr 0.0.0.0 --metrics.port ${METRICS_PORT} --log.debug"` |  |
...
| controller.annotations."secret.reloader.stakater.com/reload" | string | `"event-watcher,event-watcher-genesis"` |  |
...
| global.fullnameOverride | string | `"event-watcher"` |  |
| global.nameOverride | string | `"event-watcher"` |  |
| image.pullPolicy | string | `"Always"` |  |
| image.repository | string | `"scrolltech/event-watcher"` |  |
| image.tag | string | `"v4.4.22"` |  |
...
| initContainers.2-init-db.env[5].value | string | `"event_watcher"` |  |
| initContainers.2-init-db.env[6].name | string | `"DB_PASSWORD"` |  |
| initContainers.2-init-db.env[6].valueFrom.secretKeyRef.key | string | `"EVENT_WATCHER_PASSWORD"` |  |
...
| initContainers.5-wait-for-l1.envFrom[0].configMapRef.name | string | `"event-watcher-env"` |  |
...
| persistence.event-watcher.enabled | string | `"yes"` |  |
| persistence.event-watcher.mountPath | string | `"/app/conf/"` |  |
| persistence.event-watcher.name | string | `"rollup-node-config"` |  |
| persistence.event-watcher.type | string | `"configMap"` |  |
...
| persistence.migrate-db.name | string | `"event-watcher-migrate-db"` |  |
...
| podLabels.app | string | `"event-watcher"` |  |
